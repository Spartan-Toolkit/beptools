...
import TokenManagement from "@binance-chain/javascript-sdk/lib/client/token";
...
class Binance {
  ...
  fees() {
    return this.httpClient.get("api/v1/fees");
  }
  ...
  async lock(fromAddr, symbol, amount) {
    ...
    const signedTx = await this.bnbClient._prepareTransaction(
      lockMsg,
      lockSignMsg,
      fromAddr
    );
